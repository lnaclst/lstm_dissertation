...
# Set whether training or testing embeddings
use_speaker_embed = True
embedding_path = 'embeddings/embedding-12-7-19-2-f.pt'
model_path = '/group/project/cstr1/mscslp/2019-20/s1983536_Lena_Smith/two_subnets_base/2_Acous_10ms_Ling_50ms/test/1_2_Acous_10ms_Ling_50ms_m_50_a_50_v_50_lr_01_l2e_0001_l2o_-05_l2m_-05_l2a_0001_l2v_-05_dmo_25_dmi_5_dao_25_dai_25_dvo_0_dvi_25_seq_600/model.p'
...
#         print(y_test.size())
        ...
        if use_speaker_embed:
            ...
            for i in range(600):
                ...
                embeddings_list.append(my_embedding[0])
            embeddings_tensor = torch.stack(embeddings_list,0)
            print(embeddings_tensor.size())

            # Can I get this model prediction with a different prediction size than it was trained with????
            out_test = model(model_input, embeddings_tensor)
        else:
...
#         quit()
        ...
        loss = loss_func_BCE(F.sigmoid(out_test), y_test.transpose(0, 1))  # why are we using this function here, but the BCE with logits function below?????????  #   y_test is the data, out_test is the prediction
        # print("Sigmoid: ",F.sigmoid(out_test))
        # print("Ground truth: ", y_test.transpose(0,1))
        # print("Loss:", loss)
        # loss = loss_func_BCE_Logit(out_test,y_test.transpose(0,1))  # <- This. Why are we not using this?
...
#     print(y_test)
    ...
    loss_weighted_mean = np.sum(np.array(batch_sizes) * np.squeeze(np.array(losses_test))) / np.sum(batch_sizes)      # Averaging over all of the samples in the batches
...
#                       embedding_info=embedding_info,speaker_embed = use_speaker_embed)




# model = torch.load('model-to-load')
...
### Append embeddings to the model states here?
test()
...
#     model.train()       # xx
#     t_epoch_strt = t.time()   # xx
#     loss_list = []   # xx
#     model.change_batch_size_reset_states(train_batch_size)   # xx
#
#     if onset_test_flag:   # xx
#         # setup results_dict
...
#         model.init_hidden()   # xx
#         model.zero_grad()   # xx
#         model_input = []   # xx
#
#         for b_i, bat in enumerate(batch):   # xx
#             if len(bat) == 0:
#                 model_input.append(bat)
#             elif (b_i == 1) or (b_i == 3):
...
#         y = Variable(batch[4].type(dtype).transpose(0, 2).transpose(1, 2))   # xx
#         ##### ?????
#         info = batch[5]   # xx
#         # Prediction
#
#         # Need this is test, not train
#         if use_speaker_embed:
#             embeddings_list = []
#             for i in range(600):
#                 my_embedding = trained_embeddings[batch_indx]
...
#             embeddings_tensor = torch.stack(embeddings_list,0)
#             print(embeddings_tensor.size())
#             model_output_logits = model(model_input, embeddings_tensor)
#         else:
...
#             batch_embedding = model_output_logits                  # Saving this after the hidden
#             epoch_hidden_states.append(batch_embedding)
#             print("Batch embedding size: ", batch_embedding.size())
#
...
#     for i in embeddings:
#         if i.size() != (1,128,50):
#             print("Irregular size: ", i.size())
#     torch.save(embeddings, 'embeddings/embedding-{}-{}-{}-{}-{}.pt'.format(now.day, now.month, now.hour, now.minute,annotations_role[0]))
#
...
### Is this doing anything with test results?
