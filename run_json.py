...
# sequence_length = 800
# dropout = 0
prediction_length = 60 # who was the floorholder?      # (3 seconds of prediction)
shuffle = True
num_layers = 1
onset_test_flag = True
annotations_role = ''                  # Either an empty string or 'fg' if using f or 'gf' if using g
annotations_dir = '/group/project/cstr1/mscslp/2019-20/s1983536_Lena_Smith/data/extracted_annotations/voice_activity/'     #{}'.format(annotations_role[0])     # Location of files used as ground truth, to calculate loss
...
# structure: hold_shift.hdf5/50ms-250ms-500ms/hold_shift-stats/seq_num/g_f_predict/[index,i]
hold_shift = h5py.File('/group/project/cstr1/mscslp/2019-20/s1983536_Lena_Smith/data/datasets/hold_shift.hdf5', 'r')
...
onsets = h5py.File('/group/project/cstr1/mscslp/2019-20/s1983536_Lena_Smith/data/datasets/onsets.hdf5', 'r')
...
overlaps = h5py.File('/group/project/cstr1/mscslp/2019-20/s1983536_Lena_Smith/data/datasets/overlaps.hdf5', 'r')
...
# %% Test function
def test(onset_thresh,embedding_tensor):
    losses_test = list()
    results_dict = dict()
    losses_dict = dict()
    batch_sizes = list()
    ...
    ### Would use the model module here:
    for batch_indx, batch in enumerate(test_dataloader):
        ...
        if batch_indx == 0:
            model2.change_batch_size_reset_states(batch_length)
        else:
            if slow_test:
                model2.change_batch_size_no_reset(batch_length)
            else:
                model2.change_batch_size_reset_states(batch_length)
        ...
        if use_speaker_embed:
            ...
            out_test = model2(model_input, embedding_tensor)
        else:
            print("No embedding in test.")
            out_test = model2(model_input)
    ...
    #
            ...
            if 'f_scores_' + onset_str_list[0] not in results_save:
                results_save['f_scores_' + onset_str_list[0]] = []
            ...
            if not(len(true_vals_onset) == 0):
                ...
                tn,fp,fn,tp, = 0,0,0,0
            if 'tn_' + onset_str_list[0] not in results_save:
                results_save['tn_' + onset_str_list[0]] = []
            if 'fp_' + onset_str_list[0] not in results_save:
                results_save['fp_' + onset_str_list[0]] = []
            if 'fn_' + onset_str_list[0] not in results_save:
                results_save['fn_' + onset_str_list[0]] = []
            if 'tp_' + onset_str_list[0] not in results_save:
                results_save['tp_' + onset_str_list[0]] = []
...
if use_speaker_embed:       # If training embeddings
    ...
    for i in range(64):    ### Change this depending on the size of the embeddings
...
#
model2 = LSTMPredictor(lstm_settings_dict=lstm_settings_dict, feature_size_dict=feature_size_dict,
                      batch_size=train_batch_size, seq_length=sequence_length, prediction_length=prediction_length,
                      embedding_info=embedding_info,speaker_embed = use_speaker_embed, embedding_for_size = embedding_t)

model_path = '/group/project/cstr1/mscslp/2019-20/s1983536_Lena_Smith/full_model_split3/train_baseline_split32/2_Acous_10ms_Ling_50ms/test/1_2_Acous_10ms_Ling_50ms_m_50_a_50_v_50_lr_01_l2e_0001_l2o_-05_l2m_-05_l2a_0001_l2v_-05_dmo_25_dmi_5_dao_25_dai_25_dvo_0_dvi_25_seq_600/'

state_dict = torch.load(model_path + 'model.p')
keys = ["lstm_visual.weight_ih_l0", "lstm_visual.weight_hh_l0", "lstm_visual.bias_ih_l0", "lstm_visual.bias_hh_l0"]
rep_keys = ["lstm_cell_visual.weight_ih", "lstm_cell_visual.weight_hh", "lstm_cell_visual.bias_ih", "lstm_cell_visual.bias_hh"]

# for i in range(len(rep_keys)):
#     state_dict[rep_keys[i]] = state_dict[keys[i]]
#     del state_dict[keys[i]]

use_onset = True          # Permanently set this to True after the next experiment
if use_onset:
    with open(model_path + 'onset_thresh.txt', 'r') as o:
        for line in o:
            onset_thresh_val = float(line)
else:
    onset_thresh_val = 0
print('Onset thresh val: ', onset_thresh_val)

model2.load_state_dict(state_dict)
model2.eval()
test(onset_thresh_val,embedding_t)
...
print("Dirs: ", results_dir , result_dir_name)
