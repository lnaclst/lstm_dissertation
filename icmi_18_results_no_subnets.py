...
#This is a list of the feature dictionaries in feature_vars. This is the feature_dict_list? 
# List of lists
# Each list referenced in this list gets passed to run_trial and is the value to the key feature_dict_list in json_dict (below). json_dict is loaded by data_loader.py and feature_dict_list is used to define the trial. 
#### Does it define the data that gets used in each trial? 
...
#    for test_indx in range(number_of_tests):
    for test_indx in test_indices:
        ...
        if not(os.path.exists(os.path.join(test_path,name_append_test))) and not(os.path.exists(os.path.join(test_path,name_append_test,'results.p'))):
            ##### FEATURE DICT LIST DEFINED HERE #####
